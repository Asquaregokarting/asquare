...
import {
  setKbArticleActive,
  subscribeToTicketKbArticles,
  upsertKbArticle,
} from '../../../api/ticket-kb'
import type {
  CreateTicketKbArticlePayload,
  TicketKbArticle,
  TicketKbVisibility,
} from '../../../api/types'
import { logger } from '../../../../lib/logger'

const VISIBILITY_OPTIONS: TicketKbVisibility[] = ['internal', 'customer']
...
const draftFromArticle = (a: TicketKbArticle): DraftState => ({
  id: a.id,
  title: a.title,
  body: a.body,
...
export const TicketKbSettings = () => {
  ...
  const [rows, setRows] = useState<TicketKbArticle[]>([])
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToTicketKbArticles(
      undefined,
      (next) => {
        setRows(next)
        setLoading(false)
      ...
      (err) => {
        logger.error('ticket_kb.subscribe_failed', err)
        setLoading(false)
      },
    )
    return () => {
  ...
  const handleEdit = (a: TicketKbArticle) => {
    setValidationError(null)
    setDraft(draftFromArticle(a))
  }

  const handleToggleActive = async (a: TicketKbArticle) => {
    setTogglingId(a.id)
    try {
      await setKbArticleActive(a.id, !a.active)
      success(`${a.title} ${a.active ? 'deactivated' : 'activated'}`)
    } catch (err) {
      logger.error('ticket_kb.toggle_failed', err instanceof Error ? err : new Error(String(err)))
      toastError('Failed to toggle article')
    } finally {
      setTogglingId(null)
    }
  }
  ...
  const handleSave = async () => {
    ...
    const payload: CreateTicketKbArticlePayload = {
      title,
      body,
      tags,
    ...
    setSaving(true)
    try {
      await upsertKbArticle(id, payload)
      success(`Article ${title} saved`)
      setDraft(null)
    } catch (err) {
      logger.error('ticket_kb.save_failed', err instanceof Error ? err : new Error(String(err)))
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-display text-lg font-semibold text-text">Knowledge base articles</h2>
          <p className="text-xs text-muted">
            Internal articles power the &ldquo;/&rdquo; menu in tickets. Customer-visibility
            articles also appear in the customer Help page FAQ.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-border/45 bg-panel p-8 text-center">
          <p className="text-sm text-muted">No KB articles yet.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-border/45 bg-surface/50">
                ...
                <th className="whitespace-nowrap px-3 py-2.5 font-medium text-muted">Visibility</th>
                <th className="px-3 py-2.5 font-medium text-muted">Tags</th>
            ...
            <tbody>
              {sorted.map((a) => (
                <tr key={a.id} className="border-b border-border/30 transition hover:bg-surface/30">
                  <td className="whitespace-nowrap px-3 py-2.5 font-medium text-text">
                    {a.title}
                    <div className="font-mono text-[10px] text-muted">{a.id}</div>
                  </td>
                  <td className="whitespace-nowrap px-3 py-2.5 capitalize">{a.visibility}</td>
                  <td className="px-3 py-2.5 text-xs text-muted">{a.tags.join(', ') || '—'}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 text-xs">{a.sortOrder}</td>
                  <td className="whitespace-nowrap px-3 py-2.5">
                    <button
                      type="button"
                      disabled={togglingId === a.id}
                      onClick={() => handleToggleActive(a)}
                      className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${a.active ? 'bg-success/15 text-success' : 'bg-muted/15 text-muted'}`}
                    >
                      {togglingId === a.id ? '…' : a.active ? 'Active' : 'Inactive'}
                    </button>
                  </td>
                  <td className="whitespace-nowrap px-3 py-2.5">
                    <button
              ...
              <div>
                ...
                {!draft.id ? (
                  <p className="mt-1 text-xs text-muted">
                    ID will be generated as <code>{slugify(draft.title || 'new-article')}</code>.
                  </p>
                ) : (
                  <p className="mt-1 text-xs text-muted">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setDraft({
                        ...draft,
                        visibility: e.target.value as TicketKbVisibility,
                      })
                    }
...
export default TicketKbSettings
